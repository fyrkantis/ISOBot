...
- *"**show**"* writes out all words in this server's custom word library.
				""")
				return
			whole = re.findall("(?<!(\\+|\\*|=))(((\\/|\\\\|\\-|^) *)?(((year|month) *)?((\\d{2,4}|[1-9]) *((st|nd|rd|th) *,? *)?|(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\\w* *,? *)((month|year|of))*(\\/|\\\\|\\-| ) *){1,2}(((\\d{2,4}|[1-9])( *(st|nd|rd|th)(\\s|$))?|(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\\w*))( *(\\/|\\\\|\\-|$))?)(?!(\\d|\\+|\\*|=))", message.content, re.I)
			if len(whole) > 0:
...
@slash.slash( # TODO: Add parameter for writing out other server's word libraries.
	name = "words",
	description = "I'll write out all special words that I'm currently using from this server's custom word library.",
	options = [
		create_option(
			name = "library",
			description = "\"standard\" for default library, \"all\" for all connected libraries, or a server ID it's library.",
			option_type = 3,
			required = False
		)
	],
...
async def words(ctx, library = None):
